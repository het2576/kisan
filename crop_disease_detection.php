...
// Clear results on page load (not POST)
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["cropImage"])) {
    ...
    if(isset($_POST["submit"])) {
        ...
        if($check !== false) {
            // Additional validation for crop images
            ...
            // Sample pixels for green color dominance
            for($x = 0; $x < $width; $x += 4) {
                for($y = 0; $y < $height; $y += 4) {
                    $rgb = imagecolorat($image, $x, $y);
                    $colors = imagecolorsforindex($image, $rgb);
                    
                    // Check if pixel is in the green spectrum (typical for plants)
            ...
            // If less than 15% green, probably not a crop image
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["cropImage"]["tmp_name"], $target_file)) {
            // Simulate disease detection by randomly selecting from database
            $diseases = array_keys($disease_database);
            $detected_disease = $diseases[array_rand($diseases)];
            
            // Store the complete disease data
            $_SESSION['detection_result'] = $disease_database[$detected_disease];
                    ...
                    <?php if(isset($_SESSION['detection_result']) && isset($_SESSION['uploaded_image'])): ?>
                        ...
                        <div class="disease-card">
                            ...
                            <h3 class="text-primary mb-4">
                                <?php echo isset($_SESSION['detection_result']['name'][$lang]) ? $_SESSION['detection_result']['name'][$lang] : ''; ?>
                            </h3>
                            ...
                            <div class="treatment-card">
                                ...
                                <p class="mb-0">
                                    <?php echo isset($_SESSION['detection_result']['treatment'][$lang]) ? $_SESSION['detection_result']['treatment'][$lang] : ''; ?>
                                </p>
                            </div>
                        </div>
                    <?php else: ?>
